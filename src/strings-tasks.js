...
function sumOfCodes(str) {
  if (str) {
    return Array.from(str).reduce((acc, char) => acc + char.charCodeAt(), 0);
  }
  return 0;
}
...
function isPalindrome(str) {
  const resStr = str.toLowerCase().replace(/[^a-zа-я0-9]+/g, '');
  return resStr.split('').reverse().join('') === resStr;
}
